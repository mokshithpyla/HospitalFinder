...
class ImageHeaderScrollView extends Component<Props, State> {
  ...
  constructor(props: Props) {
    ...
    this.state = {
      ...
      foreGroundHeight: this.props.maxHeight,
      pageY: 0,
    };
    this.state.scrollY.addListener(
      ({value}) => {
        const newHeight = this.props.maxHeight - value;
        this.setState({
          foreGroundHeight: this.props.minHeight > newHeight ? this.props.minHeight : newHeight
        });
      }
    );
  }
  ...
  renderForeground() {
    ...
    const go ={
      height: this.state.foreGroundHeight
    }

    return (
      <View style={[styles.header, go]}>
